    ...
    while ($true) {
        ...
        switch ($mod_sudo) {
            "1" {
                ...
                if (Get-LocalGroupMember -Group $group_name -Member $user_name -ErrorAction SilentlyContinue) {
                    Write-Host "`nL'utilisateur $user_name fait déjà parti du groupe $group_name."
                }
                else {
            ...
            while ($true) {
                ...
                if (Get-LocalGroup | Where-Object { $_.SID -notmatch '^S-1-5-32-' } | Select-Object -ExpandProperty Name) {
                    ...
                    if (Get-LocalGroup -Name $local_grp -ErrorAction SilentlyContinue) {
                        ...
                        if (Get-LocalGroupMember -Group $local_grp -Member $user_name -ErrorAction SilentlyContinue) {
                            Write-Host "`nL'utilisateur $user_name fait déjà parti du groupe $local_grp."
                        }
                        else {
