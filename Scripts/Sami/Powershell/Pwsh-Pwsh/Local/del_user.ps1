...
while ($true) {
    ...
    switch ($del_user) {       
        "1" {  
            #ssh cliwin01 "Remove-LocalUser -Name \"$user_name\"; Remove-Item \"C:\Users\$user_name\" -Recurse -Force"
            Remove-LocalUser -Name $user_name; Remove-Item "C:\Users\$user_name" -Recurse -Force -ErrorAction SilentlyContinue
            Write-Host "`nL'utilisateur $user_name ainsi que son répertoire personnel a été suprimé !`n"
            Log "UserDeletion"
            end_user_return
            return
        }
